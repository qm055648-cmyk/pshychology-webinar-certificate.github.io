...
async function verify() {
    ...
    msg.innerText = "🔍 Verifying certificate...";
    ...
    try {
        ...
        for (let i = 1; i < rows.length; i++) {
            if (!rows[i]) continue; // empty row skip
        ...
        if (found) {
            msg.innerText = "✅ Certificate Verified Successfully";
            msg.style.color = "green";
            cert.style.display = "block";
        } else {
            msg.innerText = "❌ Invalid ID or Record Not Found";
            msg.style.color = "red";
        }

    } catch (error) {
        msg.innerText = "⚠️ Unable to connect. Please try again later.";
        msg.style.color = "orange";
        console.error(error);
    }
}
